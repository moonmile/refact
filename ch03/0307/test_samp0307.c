...
// Test: Normal case
void test_make_qrcode_url_normal(void) {
    int type = 1;
    ...
    const char *result = make_qrcode_url(type, device_id, url, encypt_key, t);
    CU_ASSERT_PTR_NOT_NULL(result);
    CU_ASSERT_STRING_EQUAL(result, "https://simple.com/qrcode?msWBgyUw2s99TZP1vdX3j6BMbnvB57WHpfn5eIM7w");
}

// Test: Invalid key length
void test_make_qrcode_url_invalid_key(void) {
    int type = 1;
    const char *device_id = "XXXXX-01234567";
    const char *url = "https://simple.com/qrcode?";
...
void test_make_qrcode_url_diff_device(void) {
    int type = 2;
...
// Test: Null device_id
void test_make_qrcode_url_null_device(void) {
    int type = 1;
...
// Test: Null URL
void test_make_qrcode_url_null_url(void) {
    int type = 1;
    const char *device_id = "XXXXX-01234567";
    const char *url = NULL;
    const char *encypt_key = "560DB2A64C2BBE61FBE6810AFCB25230";
...
int main() {
    ...
    CU_pSuite suite = CU_add_suite("make_qrcode_url Suite", NULL, NULL);
    CU_add_test(suite, "Normal case", test_make_qrcode_url_normal);
    CU_add_test(suite, "Invalid key", test_make_qrcode_url_invalid_key);
    CU_add_test(suite, "Different device", test_make_qrcode_url_diff_device);
    CU_add_test(suite, "Null device_id", test_make_qrcode_url_null_device);
    CU_add_test(suite, "Null url", test_make_qrcode_url_null_url);
